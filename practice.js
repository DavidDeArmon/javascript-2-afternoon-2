...
//Returns function of only evens
const evenFinder = nums =>nums.filter(nums=>nums%2==0)
...
const divider = numbersArray =>{
  const evenArray = numbersArray.filter(num =>num%2===0)
  const oddArray = numbersArray.filter(num => num%2===1)
  return dividerArray = [evenArray,oddArray]
}
...
const finder = arr =>{
  ...
  arr.forEach(element => element===randomNumber? foundNumber=true:false)
  return foundNumber;
}
...
const removeItem = (myGroceryList,newItem) => {
  if(!myGroceryList){return []}
  else if(!newItem){return []}
  return myGroceryList.filter(item=> item != newItem)

  
}
const addItem =(myGroceryList,newItem) =>{
  if(!myGroceryList){return []}
  else if(!newItem){return[]}
...
const both = (arr1,arr2) => {
  return arr1.filter(element1=>{
    ...
    for(let i = 0;i<arr2.length;i++){
      element1===arr2[i]?bool=true:false
    }
    return bool;
  })
}
